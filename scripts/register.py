#!/usr/bin/env python3
"""纯协议 ChatGPT 注册（Outlook / URL 接码邮箱）。
...
接码账号格式（用 ---- 分隔）：
    ...
    email----https://assurivo.com/console/open.php?mail=...&pwd=...
...
from app.integrations.mail.providers.outlook import OutlookMailProvider  # noqa: E402
from app.integrations.mail.providers.mailbox_url import MailboxURLProvider  # noqa: E402
...
def main():
    if len(sys.argv) < 2:
        print(
            ...
            "   or: python register_outlook.py "
            "'email----https://assurivo.com/console/open.php?mail=...&pwd=...'",
            file=sys.stderr,
        )
        sys.exit(2)

    parts = sys.argv[1].split("----", 3)
    if len(parts) not in (2, 4):
        print(f"格式错: 需要 2 段或 4 段，拿到 {len(parts)} 段", file=sys.stderr)
        sys.exit(2)
    if len(parts) == 2:
        email, access_url = parts
        mail = MailboxURLProvider(email=email, access_url=access_url)
        logger.info("账号: %s  邮箱来源=通用接码地址", email)
    else:
    ...
    flow = AuthFlow(cfg)
    logger.info("[auth_flow] run_register 启动 (纯协议 + 邮箱验证码) ...")
    partial = False
    try:
