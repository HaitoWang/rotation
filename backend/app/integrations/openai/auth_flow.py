...
from app.integrations.openai.fingerprint import generate_fingerprint, ua_for_impersonate, fingerprint_for_impersonate
from app.integrations.mail.providers import MailProvider
from app.integrations.openai.http_client import create_http_session, USER_AGENT

logger = logging.getLogger(__name__)

# 出口代理固定美国。所有「还没探到国家码」的路径都用这个兜底，
# 绝不能让指纹落到 UTC 时区 / 非美语言——那与美国住宅 IP 自相矛盾，
# 会被 sentinel 静默标记后延迟死号。可用 AUTH_GEO_COUNTRY 覆盖。
_DEFAULT_GEO_COUNTRY = (os.getenv("AUTH_GEO_COUNTRY", "") or "US").strip().upper() or "US"

# 地理锁：即使 cloudflare trace 报了别的国家码，也不跟着换语言/时区。
# 设 AUTH_GEO_LOCK=0 才允许跟随实际出口（换非美国代理池时才需要关）。
_GEO_LOCK = (os.getenv("AUTH_GEO_LOCK", "1") or "1").strip().lower() not in ("0", "false", "no", "off")
...
class AuthFlow:
    ...
    def __init__(
        ...
        on_authenticated_session: Optional[Any] = None,
        ...
        # 代理出口固定美国：初始指纹就按 US 生成，不要先出 UTC 再等 check_proxy 纠正。
        # 中间任何一个请求带着 UTC 时区上线都是自相矛盾。
        self._fingerprint = generate_fingerprint(country_code=_DEFAULT_GEO_COUNTRY)
        ...
        self.session = self._new_session(
            self._impersonate_candidates[self._impersonate_idx]
        )
        ...
        self._should_stop = (env_overrides or {}).get("_should_stop")
        self._email_phase_lock = (env_overrides or {}).get("_email_phase_lock")
        self._email_phase_lock_acquired = False
        if self._sms_callback is not None:
            try:
                self._sms_callback.should_stop = self._should_stop
            except Exception:
                pass
        # 密码一在 OpenAI 侧生效就回调出去，调用方负责立刻落盘。
        # 签名 (email: str, password: str) -> None，异常由 register_password 吞掉。
        # 协议层不认识数据库，所以只给回调，"存哪"留给调用方决定，
        ...
        # 完成邮箱登录并取得稳定 ChatGPT session 后、Codex OAuth 前的接管钩子。
        # 返回 AuthResult/dict 时表示调用方已经完成 Codex 授权，本流程跳过默认交换。
        self._on_authenticated_session = on_authenticated_session
        self._authenticated_snapshot_branch = False
    ...
    def _cookie_snapshot(self) -> list[dict]:
        """Capture the full cookie jar without sharing a libcurl/session object."""
        records: list[dict] = []
        try:
        ...
        try:
            for name, value in self.session.cookies.items():
                records.append({"name": name, "value": value, "domain": "", "path": "/", "secure": False})
        except Exception:
            pass
        return records

    def fork_authenticated_for_codex(
        self,
        *,
        sms_callback: Optional[Any] = None,
        env_overrides: Optional[dict] = None,
    ) -> "AuthFlow":
        """Create an isolated Codex flow from a completed ChatGPT login."""
        child = AuthFlow(
            self.config,
            sms_callback=sms_callback,
            env_overrides=dict(env_overrides or self._env_overrides),
        ...
        try:
            ...
            pass
        child._country_code = self._country_code
        child._fingerprint = copy.deepcopy(self._fingerprint)
        child._ua = self._ua
        child._impersonate_candidates = list(self._impersonate_candidates)
        child._impersonate_idx = self._impersonate_idx
        child.session = child._new_session(
            child._impersonate_candidates[child._impersonate_idx]
        )
        for record in self._cookie_snapshot():
            kwargs = {
            ...
            if record.get("expires") is not None:
                kwargs["expires"] = record["expires"]
            try:
                child.session.cookies.set(record["name"], record["value"], **kwargs)
            except TypeError:
                # curl_cffi and requests expose compatible jars but differ on
                # optional cookie kwargs in older versions.
                child.session.cookies.set(
                    record["name"], record["value"], domain=record["domain"], path=record["path"]
                )
        for field in (
            "email", "password", "session_token", "access_token", "device_id",
            "csrf_token", "cookie_header", "totp_secret",
        ):
            setattr(child.result, field, getattr(self.result, field, ""))
        # Every branch starts a fresh state/PKCE transaction. Do not inherit any
        # callback verifier, auth dump or Sentinel token from the registration flow.
        child._manual_login_verifier = ""
        child._captured_login_verifier = ""
        child._dump_login_verifier = ""
        child._oauth_state = ""
        child._oauth_auth_url = ""
        child._client_auth_session_dump = {}
        child._client_auth_session_id = ""
        child._codex_rt_attempted = False
        child._last_sentinel_token = ""
        child._last_sentinel_so_token = ""
        child._authenticated_snapshot_branch = True
        return child

    def run_codex_from_authenticated_session(self) -> AuthResult:
        """Run only Codex OAuth; never fall back to another email login."""
        if not (self.result.session_token and self.result.access_token):
            raise RuntimeError("认证快照缺少 session_token/access_token")
        try:
            self.fetch_client_auth_session_dump("post_login_fork")
        except Exception as exc:
            logger.debug("认证快照分支未取到 client-auth dump，继续 OAuth: %s", exc)
        if not self.oauth_codex_rt_exchange(mail_provider=None, allow_login_fallback=False):
            raise RuntimeError("Codex 分支未获取 refresh_token")
        if not self.result.refresh_token:
            raise RuntimeError("Codex 分支交换成功但 refresh_token 为空")
        # Restore the browser-session AT after the token endpoint temporarily
        # stores the Codex AT in result.access_token.
        try:
            self.get_auth_session()
        except Exception as exc:
            # The parent already supplied a validated ChatGPT session/access
            # token. A post-exchange refresh failure must not discard a valid RT.
            logger.warning("Codex 分支换 RT 后刷新 ChatGPT session 失败，保留认证快照: %s", exc)
        return self.result

    def _raise_if_stopped(self, stage: str = "流程") -> None:
        """在会话级竞速中尽快终止已经失去胜负资格的分支。"""
        if callable(self._should_stop):
            try:
                stopped = bool(self._should_stop())
            except Exception:
                stopped = False
            if stopped:
                raise RuntimeError(f"{stage}因竞速对端完成/任务停止而中止")

    def _acquire_email_phase_lock(self) -> None:
        lock = self._email_phase_lock
        if lock is None or self._email_phase_lock_acquired:
            return
        while True:
            self._raise_if_stopped("邮箱验证码阶段")
            if lock.acquire(timeout=0.25):
                self._email_phase_lock_acquired = True
                return

    def _release_email_phase_lock(self) -> None:
        if not self._email_phase_lock_acquired:
            return
        self._email_phase_lock_acquired = False
        try:
            self._email_phase_lock.release()
        except RuntimeError:
            pass
    ...
    def _wait_for_mail_otp(
        ...
        *,
        ...
        """Race 模式下把阻塞式邮箱读取包装成可协作取消的等待。"""
        if not self._env_flag("CANCELLABLE_MAIL_WAIT", "0"):
            return mail_provider.wait_for_otp(
                email, timeout=timeout, issued_after=issued_after,
            )
        result_queue: queue.Queue = queue.Queue(maxsize=1)

        def _read() -> None:
            try:
                result_queue.put((True, mail_provider.wait_for_otp(
                    email, timeout=timeout, issued_after=issued_after,
                )))
        ...
        threading.Thread(
            target=_read,
            daemon=True,
            name=f"mail-otp-{id(self):x}",
        ).start()
        while True:
            self._raise_if_stopped("邮箱验证码等待")
            try:
    ...
    def _build_chatgpt_cookie_header(self) -> str:
        ...
        for name in critical_names:
            if name in seen:
                continue
            try:
                value = self._get_cookie_value_by_name(name, host="chatgpt.com")
            except Exception:
                value = ""
            if value:
                seen.add(name)
    ...
    def _get_cookie_value_by_name(
        ...
        *,
        ...
        """Read one cookie deterministically, optionally scoped to a host.

        curl_cffi raises CookieConflict when ``cookies.get(name)`` sees
        different values on sibling domains. OpenAI legitimately sets
        ``oai-did`` on both ``.chatgpt.com`` and ``.openai.com``, so request
        code must select the value that the target host would actually send.
        """
        try:
            cookies = getattr(self.session, "cookies", None)
            if cookies is None:
                return ""
            # curl_cffi exposes ``Cookies.jar`` while requests exposes the
            # CookieJar itself. Both contain Cookie objects and share the same
            # domain/path semantics.
            ...
            def value_for_host(request_host: str) -> str:
                request_host = (request_host or "").strip().lower().lstrip(".").split(":", 1)[0]
                if not request_host:
                    return ""
                matches = []
                for cookie in candidates:
                    domain = (getattr(cookie, "domain", "") or "").strip().lower().lstrip(".")
                    if domain and (request_host == domain or request_host.endswith("." + domain)):
                        path = (getattr(cookie, "path", "") or "/").strip()
                        matches.append((request_host == domain, len(domain), len(path), cookie))
                if not matches:
                    return ""
                matches.sort(key=lambda item: item[:3], reverse=True)
                return (getattr(matches[0][3], "value", "") or "").strip()

            for request_host in (host, *fallback_hosts):
                value = value_for_host(request_host)
                if value:
                    return value
            if not host and not fallback_hosts and candidates:
                return (getattr(candidates[0], "value", "") or "").strip()
        except Exception:
            pass
        return ""
    ...
    def _extract_login_challenge_from_cookie(self) -> str:
        ...
        """
        raw = self._get_cookie_value_by_name("login_session", host="auth.openai.com")
        if not raw:
            return ""
        try:
            p0 = raw.split(".")[0]
    ...
    def _follow_authorize_for_callback(self, start_url: str, redirect_uri: str, trace_prefix: str) -> tuple[str, str]:
        ...
        for i in range(12):
            if self._callback_has_code(current, redirect_uri):
                ...
                break
            # 重定向 hop 也必须带完整指纹头：只给 Accept/Referer/UA 的话，
            # curl_cffi 会替我们补上内建的 sec-ch-ua-platform: "macOS" 和默认语言，
            # 与 Windows UA + 美国出口自相矛盾。
            hop_headers = self._navigation_headers("https://chatgpt.com/")
            hop_headers["Sec-Fetch-Site"] = "cross-site"
            resp = self.session.get(
                current,
                headers=hop_headers,
                timeout=30,
    ...
    def _codex_drive_login_from_log_in(self, mail_provider: Optional[MailProvider] = None) -> str:
        ...
        if not password:
            password = self._get_env("LOGIN_PASSWORD", "").strip()
        if not password:
            password = self._get_env("REGISTER_PASSWORD", "").strip()
        if not password and self._account_callback:
            cred = self._account_callback(email) or {}
            password = (cred.get("password") or "").strip()
        if not password:
            raise RuntimeError("existing_account_missing_password")
        self.result.password = password

        device_id = (self.result.device_id or "").strip() or self._get_cookie_value_by_name(
            "oai-did", host="auth.openai.com", fallback_hosts=("chatgpt.com",)
        )
        if not device_id:
        ...
        if need_otp:
            ...
            if not self.kickoff_otp_delivery("codex_login_need_otp"):
                self.send_otp()
            self._acquire_email_phase_lock()
            try:
                otp_code = self._wait_for_mail_otp(
                    mail_provider, email,
                    timeout=otp_timeout,
                ...
                self._release_email_phase_lock()
        ...
        if self._sms_callback is not None:
            try:
                ...
                try:
                    ...
                    pass
                # A post-login race branch is pinned to one provider. Falling
                # through to a shared environment phone would violate branch
                # isolation and can let a cancelled loser continue working.
                env_phone = (os.getenv("OPENAI_PHONE_NUMBER", "") or "").strip()
                if getattr(self, "_authenticated_snapshot_branch", False) or not env_phone:
                    raise
    ...
    def _handle_add_phone_via_sms(self, continue_url: str = "") -> str:
        ...
        换号次数由 sms_max_phone_attempts 控制（默认 3），达到上限即结束本任务。
        """
    ...
    def _do_sms_loop(self, ctrl) -> str:
        ...
        logger.info(
            "[sms] 配置: provider=%s 单号窗口=%ds 最多尝试手机号=%d 单号内验证重试=%d",
        ...
        _PHONE_REJECTED_PATTERNS = (
            ...
            "suspicious behavior from phone", "invalid phone", "already in use",
            "not valid", "voip",  # OpenAI 风控：号段可疑/虚拟号
        )
        ...
        while phone_attempt < max_phone_attempts:
            if callable(self._should_stop) and self._should_stop():
            ...
            try:
                ...
                retry_after = max(1.0, min(300.0, float(getattr(e, "retry_after", 1.0))))
                jitter = random.uniform(0.0, min(2.0, retry_after * 0.2))
                logger.info("[sms] 接码资源暂不可用，%.1fs 后重试", retry_after + jitter)
                deadline = time.time() + retry_after + jitter
                while time.time() < deadline:
                    if callable(self._should_stop) and self._should_stop():
                        raise RuntimeError("SMS 接码因任务停止而中止")
            ...
            while time.time() - phone_start < per_phone_timeout and code_attempt < max_code_retries_per_phone:
                ...
                if callable(self._should_stop) and self._should_stop():
                    raise RuntimeError("SMS 等码因竞速对端完成/任务停止而中止")
                if not code:
                    break  # 超时换号
        ...
        raise RuntimeError(
            f"SMS 接码达到最多手机号尝试次数 {max_phone_attempts}，仍未完成"
        ) from last_err
    ...
    def oauth_codex_rt_exchange(
        ...
        *,
        ...
        try:
            ...
            if (not callback_url) and "/log-in" in (final_url or "") and not allow_login_fallback:
                logger.warning("Codex 分支认证快照已失效，禁止回落到邮箱登录")
                return False
    ...
    def _extract_hydra_redirect_values(self) -> list[str]:
        ...
        raw = self._get_cookie_value_by_name("hydra_redirect", host="auth.openai.com")
        if not raw:
            return []
        out: list[str] = []
        try:
    ...
    def _collect_code_verifier_candidates(self, callback_url: str, continue_url: str) -> list[tuple[str, str]]:
        ...
        raw_candidates: list[tuple[str, str]] = [
            ...
            ("cookie_login_verifier", self._get_cookie_value_by_name(
                "login_verifier", host="auth.openai.com"
            )),
            ("cookie_code_verifier", self._get_cookie_value_by_name(
                "code_verifier", host="auth.openai.com"
            )),
            ("cookie_login_challenge", self._extract_login_challenge_from_cookie()),
            ("cookie_nextauth_state", self._get_cookie_value_by_name(
                "__Secure-next-auth.state", host="chatgpt.com"
            )),
        ]
    ...
    def _rotate_impersonate_session(self) -> bool:
        ...
        self._ua = self._fingerprint.get("user_agent") or ua_for_impersonate(imp, self._ua)
        logger.warning(f"TLS 异常，切换指纹重试: impersonate={imp}, ua={self._ua[:60]}...")
        try:
            self.session.close()
        except Exception:
            pass
        self.session = self._new_session(imp)
        return True

    @staticmethod
    def _is_restartable_auth_session_error(exc: Exception) -> bool:
    ...
    def _restart_auth_bootstrap_session(self, email: str) -> None:
        """Discard a stale OAuth state before retrying the pre-signup bootstrap."""
        try:
            self.session.close()
        except Exception:
            pass
        # 重建指纹时必须沿用已探测到的国家码（没探到就用美国兜底），
        # 否则重启一次就退回 UTC 时区，与美国出口 IP 矛盾。
        self._fingerprint = generate_fingerprint(
            country_code=self._country_code or _DEFAULT_GEO_COUNTRY
        )
        self._ua = self._fingerprint["user_agent"]
        self._impersonate_candidates = self._fingerprint.get(
            "fallback_impersonates",
        ...
        self._impersonate_idx = 0
        self.session = self._new_session(self._impersonate_candidates[0])
    ...
    def _bootstrap_signup(self, email: str) -> tuple[str, bool]:
        """Start OAuth and retry only when the server explicitly invalidates state."""
        try:
            attempts = max(1, int(self._get_env("AUTH_BOOTSTRAP_ATTEMPTS", "3")))
        except Exception:
            attempts = 3
        for attempt in range(1, attempts + 1):
            try:
                ...
                sentinel = self.get_sentinel_token(device_id)
                return auth_url, self.signup(email, sentinel)
            except Exception as exc:
                if not self._is_restartable_auth_session_error(exc) or attempt >= attempts:
                    raise
                logger.warning(
                    "OAuth 会话失效，第 %d/%d 次从 CSRF 开始重建: %s",
                    attempt,
                    attempts,
                    exc,
                )
                self._restart_auth_bootstrap_session(email)
                self.check_proxy()
                if not self.warmup():
                    raise RuntimeError("OAuth bootstrap 重建后 warmup 未通过")
        raise RuntimeError("OAuth bootstrap 未完成")
    ...
    def _client_hint_headers(self) -> dict:
        """按当前指纹产出 Chromium Client Hints 全套（非 Chromium 家族返回空 dict）。

        单独抽出来，是为了让「绕过 _common_headers 的请求」（重定向 hop、
        token 交换）以及 Session 级默认头都能用同一套值——curl_cffi 的
        impersonate 会替我们填 sec-ch-ua-platform: "macOS"，跟 Windows UA 直接矛盾。
        """
        ...
        # 真实 HAR 里 full-version 和 full-version-list 同时出现，且带真实构建号
        for key, fp_key in (
            ...
            if fp.get(fp_key):
                headers[key] = fp[fp_key]
        # model 桌面是空串 ""，上面的真值判断会跳过——但真实 Chrome 会发空串
        if fp.get("sec_ch_ua_model") == '""' or fp.get("sec_ch_ua_model") == "":
            if fp.get("sec_ch_ua_platform") == '"Windows"':
                headers["sec-ch-ua-model"] = '""'
        return headers

    def _new_session(self, impersonate: str):
        """按当前指纹创建会话，并把语言 / Client Hints 钉进 Session 级默认头。

        这样即使某个请求忘了走 _common_headers，也不会漏出 curl_cffi 内建的
        macOS 提示或 en-US 之外的语言。
        """
        return create_http_session(
            proxy=self.config.proxy,
            impersonate=impersonate,
            user_agent=self._ua,
            accept_language=self._fingerprint.get("lang_full") or "en-US,en;q=0.9",
            client_hints=self._client_hint_headers(),
        )
    ...
    def _navigation_headers(self, referer: str = "https://chatgpt.com/") -> dict:
        """构造真实顶层导航头，统一 Client Hints 与 Sec-Fetch 语义。"""
        headers = self._common_headers(referer)
        headers.update({
    ...
    def warmup(self) -> bool:
        """GET 首页并确认 oai-did，失败时有限重试而不是继续浪费邮箱。"""
        try:
            attempts = max(1, min(4, int(self._get_env("AUTH_WARMUP_ATTEMPTS", "2"))))
        except Exception:
            attempts = 2
        try:
            timeout = max(8, min(45, int(self._get_env("AUTH_WARMUP_TIMEOUT", "25"))))
        except Exception:
            timeout = 25
        for attempt in range(1, attempts + 1):
            try:
                ...
                device_id = self._get_cookie_value_by_name(
                    "oai-did", host="chatgpt.com", fallback_hosts=("auth.openai.com",)
                )
                if device_id:
                    logger.info("chatgpt.com warmup 完成 (attempt=%d status=%d)", attempt, status)
                    return True
                logger.warning(
                    "Cloudflare warmup 未拿到 oai-did (attempt=%d/%d status=%d)",
                    attempt, attempts, status,
                )
            except Exception as e:
                logger.warning("Cloudflare warmup 失败 (attempt=%d/%d): %s", attempt, attempts, e)
            if attempt < attempts:
                if not self._rotate_impersonate_session():
    ...
    def check_proxy(self) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                #
                # AUTH_GEO_LOCK（默认开）：代理池是美国专线，出口只应该是 US。
                # 万一 loc 报了别的国家（代理商串线/中转节点），跟着它生成
                # zh-CN + Asia/Shanghai 的指纹反而更危险——那会让语言、时区、
                # navigator.languages 全部跟着漂走。锁死 US 至少保证各层自洽。
                if country_code and country_code != _DEFAULT_GEO_COUNTRY and _GEO_LOCK:
                    logger.warning(
                        "代理出口国家码为 %s，与锁定的 %s 不一致；指纹继续按 %s 生成"
                        "（避免语言/时区跟着漂移）",
                        country_code, _DEFAULT_GEO_COUNTRY, _DEFAULT_GEO_COUNTRY,
                    )
                    country_code = _DEFAULT_GEO_COUNTRY
                if country_code and country_code != self._country_code:
                    ...
                    try:
                        ...
                        pass
                    self.session = self._new_session(new_imp)
            else:
    ...
    def auth_oauth_init(self, auth_url: str) -> str:
        ...
        headers = self._navigation_headers("https://chatgpt.com/auth/login")
        resp = self.session.get(auth_url, headers=headers, timeout=30, allow_redirects=True)
        self._trace_http("auth_oauth_init", resp)

        # auth.openai.com and chatgpt.com can both set oai-did. Select the
        # cookie for the OAuth host instead of doing an ambiguous name-only get.
        device_id = self._get_cookie_value_by_name(
            "oai-did", host="auth.openai.com", fallback_hosts=("chatgpt.com",)
        )
    ...
    def get_sentinel_token(self, device_id: str) -> str:
        ...
        from app.integrations.openai.sentinel import get_sentinel_token
    ...
    def register_password(self, email: str) -> bool:
        ...
        # 两个独立注册会话竞争同一邮箱时必须提交同一个密码。
        password = self._get_env("REGISTER_PASSWORD", "").strip() or self._random_password()
        self.result.password = password
        ...
        if self.result.device_id:
            try:
                from app.integrations.openai.sentinel import get_sentinel_token as _get_st
    ...
    def _choose_account_select(self, html_text: str, current_url: str) -> str:
        ...
        for method, url, body, kind in candidates:
            try:
                ...
                # print 到 stdout 让 worker 日志能看到每个候选的具体结果
                print(
    ...
    def follow_redirect_chain(self, start_url: str) -> tuple[str, str]:
        ...
        for i in range(max_hops):
            headers = self._navigation_headers(referer)
            resp = self.session.get(
    ...
    def _reauthorize_for_session(self, original_auth_url: str) -> str | None:
        ...
        try:
            ...
            resp = self.session.get(
                authorize_url,
                headers=self._navigation_headers("https://chatgpt.com/"),
                allow_redirects=False,
                timeout=15,
            )
    ...
    def _extract_session_cookie(self) -> str:
        ...
        # 路径1：直接 get
        try:
            v = self._get_cookie_value_by_name(target, host="chatgpt.com")
            if v:
                return v
        except Exception:
            pass
        ...
        try:
            return self._get_cookie_value_by_name(target, host="chatgpt.com")
        except Exception:
            return ""
            ...
            for hop in range(8):
                ...
                if "chatgpt.com" in (parsed.netloc or "") and "/api/auth/callback" not in current:
                    ...
                    try:
                        ...
                        pass
                    break
            return bool(self._get_cookie_value_by_name(
                "__Secure-next-auth.session-token", host="chatgpt.com"
            ))
        except Exception as e:
    ...
    def oauth_token_exchange(self, callback_url: str, continue_url: str) -> bool:
        ...
        # 原来这里连 User-Agent 都没带，等于把 curl_cffi 的内建头（含 macOS 提示）
        # 直接暴露在最关键的 token 交换请求上。统一走 _common_headers。
        headers = self._common_headers(
            "https://auth.openai.com/sign-in-with-chatgpt/codex/consent"
        )
        headers["Content-Type"] = "application/x-www-form-urlencoded"
    ...
    def run_register(self, mail_provider: MailProvider) -> AuthResult:
        """执行完整注册流程"""
        self._raise_if_stopped("注册")
        # 检查网络
        if not self.check_proxy():
            logger.warning("网络预检查未通过，继续尝试注册链路以获取精确错误...")
        if not self.warmup():
            raise RuntimeError("注册前 warmup 未通过，已停止本次流程")
        self._raise_if_stopped("warmup")

        # 创建邮箱
        email = mail_provider.create_mailbox()
        self.result.email = email
        on_email_created = self._env_overrides.get("_on_email_created")
        if callable(on_email_created):
            try:
                on_email_created(email)
            except Exception as exc:
                logger.warning("更新实际邮箱运行记录失败: %s", exc)
        self._raise_if_stopped("邮箱创建")

        # 登录/注册链路。409 invalid_state 表示服务端 OAuth state 已失效，旧
        # cookie/state 上继续 POST 没有意义；在注册密码等副作用发生前可安全地
        # 丢弃整套会话并从 CSRF 开始重建。
        self._acquire_email_phase_lock()
        auth_url, is_new = self._bootstrap_signup(email)
        self._raise_if_stopped("邮箱提交")
        ...
        if is_new or self._existing_email_verification_mode == "passwordless_signup":
            ...
            try:
                ...
                if "401" in str(e):
                    ...
                    self._raise_if_stopped("邮箱验证码重试")
                    self.verify_otp(otp_code)
            ...
            if page_type == "login_password":
                ...
                # Per-flow overrides are concurrency-safe; os.environ is shared
                # by all 300 workers and must not decide this account's password.
                login_password = self._get_env("LOGIN_PASSWORD", "").strip()
                if not login_password:
                    login_password = self._get_env("REGISTER_PASSWORD", "").strip()
                ...
                if self._is_mfa_challenge_state(login_page_type, continue_url):
                    ...
                    if not totp_secret and self._account_callback:
                        ...
                        try:
                            ...
                            logger.warning(f"account_callback 异常: {e}")
                    if not totp_secret:
                        logger.warning("进入 mfa-challenge 但没有 totp_secret，终止该账号")
                        raise RuntimeError("mfa_challenge_missing_totp_secret")
                    else:
                        challenge_id = continue_url.split("/")[-1] if "/mfa-challenge/" in continue_url else ""
                        if challenge_id:
                            totp_code = _totp_now(totp_secret)
                ...
                try:
                    ...
                    if any(code in str(e) for code in ("401", "409")):
                        ...
                        self._raise_if_stopped("邮箱验证码验证重试")
                        otp_resp = self.verify_otp(otp_code)
        ...
        if continue_url:
            ...
            if (
                ...
                and self._on_authenticated_session is None
                and self._env_flag("OAUTH_CODEX_RT_BEFORE_CALLBACK", "1")
            ):
                self._raise_if_stopped("Codex OAuth")
        ...
        if self._on_session_ready is not None and self.result.access_token:
            self._raise_if_stopped("会话回调")
            try:
                self._on_session_ready(self, self.result.access_token)
            except Exception as e:
                logger.warning(f"session_ready 回调失败（不影响注册）: {e}")
                # TOTP activation is already live remotely.  Do not swallow a
                # failed one-time-secret persistence callback, or the account
                # would proceed into OAuth with an unrecoverable MFA state.
                if getattr(e, "fatal", False):
                    raise

        # Gmail/relay URL 只能串行消费最新验证码；在这里接管 Codex 授权，
        # 让 registrar 从一次邮箱登录快照派生独立平台分支。
        authenticated_handler_result = None
        if self._on_authenticated_session is not None and self.result.access_token:
            self._raise_if_stopped("认证会话接管")
            authenticated_handler_result = self._on_authenticated_session(self)
            if authenticated_handler_result is not None:
                if isinstance(authenticated_handler_result, AuthResult):
                    self.result = authenticated_handler_result
                elif isinstance(authenticated_handler_result, dict):
                    for key, value in authenticated_handler_result.items():
                        if hasattr(self.result, key) and value is not None:
                            setattr(self.result, key, value)

        # Codex OAuth refresh_token 交换（独立 authorize 链路，不依赖上面 callback 的 code）
        if (authenticated_handler_result is None) and (callback_url or continue_url):
        ...
        elif self._env_flag("REQUIRE_REFRESH_TOKEN", "0") and not self.result.refresh_token:
            raise RuntimeError("Codex OAuth 未获取 refresh_token（add-phone 未完成或交换失败）")

        logger.info("注册流程完成!")
        self._raise_if_stopped("注册完成确认")
        return self.result
    ...
    def run_protocol_login(self, mail_provider: MailProvider, email: str, password: str = "") -> AuthResult:
        ...
        if not self.warmup():
            raise RuntimeError("登录前 warmup 未通过，已停止本次流程")
        ...
        if not login_password and self._account_callback:
            cred = self._account_callback(email) or {}
            login_password = (cred.get("password") or "").strip()
        if not login_password:
            raise RuntimeError("existing_account_missing_password")
        self.result.password = login_password
    ...
    def from_existing_credentials(
        ...
        if session_token:
            ...
            try:
                ...
                new_session_token = self._get_cookie_value_by_name(
                    "__Secure-next-auth.session-token", host="chatgpt.com"
                )
                if new_access_token:
            ...
            try:
                ...
                if isinstance(user_obj, dict):
                    detected_email = detected_email or (user_obj.get("email", "") or "")
                session_token = self._get_cookie_value_by_name(
                    "__Secure-next-auth.session-token", host="chatgpt.com"
                )
                if session_token:
