...
from app.integrations.openai.config import Config  # noqa: E402
from app.integrations.openai.auth_flow import AuthFlow  # noqa: E402

logger = logging.getLogger("two_factor")


class ActivationPersistenceError(RuntimeError):
    """TOTP is active remotely but its one-time secret could not be persisted."""
...
def _enroll_and_activate(
    flow: AuthFlow,
    at: str,
    on_activated: Callable[[str, str], None] | None = None,
) -> dict | None:
    ...
    # The server-side factor is live at this exact boundary.  Persist the
    # one-time secret before sleep/verification or any later OAuth/SMS work.
    # A persistence error must propagate: continuing would create an MFA-locked
    # account whose secret exists only in volatile memory.
    if on_activated is not None:
        try:
            on_activated(secret, factor_id)
    ...
    try:
        ...
        logger.warning("[2fa] secret 已持久化，但 mfa_info 复核异常: %s", exc)

    return {"secret": secret, "factor_id": factor_id, "session_id": session_id}


# ── 快路径：复用注册会话，不重新登录 ──────────────────────────────
def bind_totp_2fa_inline(
    flow: AuthFlow,
    access_token: str = "",
    on_activated: Callable[[str, str], None] | None = None,
) -> dict | None:
    ...
    try:
        ...
        if not at:
            ...
            return None
        return _enroll_and_activate(flow, at, on_activated=on_activated)
