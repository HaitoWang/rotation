...
// GPU 串按浏览器家族返回；调用发生在运行时（document.createElement），
// 那时 bt / isSafariFamily 等常量已初始化。
function webglStrings() {
  if (isIOS) {
  ...
  if (isFirefox) {
    // Firefox on Windows 走 ANGLE，但 vendor 报 Google Inc. 而非带括号后缀
    return {
      vendor: 'Google Inc.',
...
function canvasElement() {
  ...
  el.getContext = (kind) => {
    ...
    return {
      ...
      getParameter(p) {
        // GPU 串必须跟浏览器家族一致：ANGLE/D3D11 只可能出现在 Windows Chromium。
        // Safari 报 Apple GPU，Firefox 不走 ANGLE。写死 Intel ANGLE 会与
        // Mac/iOS UA 直接矛盾。
        const gpu = webglStrings();
        if (p === dbg.UNMASKED_VENDOR_WEBGL || p === 0x1F00) return gpu.vendor;
        if (p === dbg.UNMASKED_RENDERER_WEBGL || p === 0x1F01) return gpu.renderer;
        return 0;
      },
...
// 时区真值来自进程的 TZ 环境变量（Python 侧按指纹注入），这样
// `""+new Date` —— sdk.js 真正采集的那个字段 —— 会渲染成美国偏移。
// 下面这个 Intl 补丁只是兜底（sdk.js 当前不读 Intl，但 SDK 可能升级）。
const targetTz = String(input.timezone || process.env.TZ || 'America/New_York');
...
const bt = String(input.browser_type || '');
const isIOS = bt === 'ios_safari' || /iphone/i.test(String(input.user_agent || ''));
const isSafariFamily = isIOS || bt === 'mac_safari';
const isFirefox = bt === 'firefox' || /firefox/i.test(String(input.user_agent || ''));
...
const navigatorObj = {
  ...
  productSub: '20030107',
  vendorSub: '',
  // Safari/Firefox 不暴露 navigator.connection（Chromium 专有）；
  // 桌面宽带 rtt 通常 50-100，iPhone 蜂窝/WiFi 才像 4g。
  connection: isSafariFamily || isFirefox
    ? undefined
    : { effectiveType: '4g', rtt: 50, downlink: 10, saveData: false },
  // Chrome/Safari 桌面内置 PDF 插件计 5 项；iOS Safari 无插件；Firefox 现代版为 0
  plugins: { length: isIOS || isFirefox ? 0 : 5 },
  mimeTypes: { length: isIOS || isFirefox ? 0 : 2 },
...
const context = {
  ...
  screen: {
    ...
    height: screenH,
    availWidth: screenW,
    // Windows 任务栏占 40px；macOS 菜单栏占 25px；iOS 全屏
    availHeight: isIOS ? screenH : (isSafariFamily ? screenH - 25 : screenH - 40),
    colorDepth: 24,
    pixelDepth: 24,
    // iPhone 竖屏；桌面横屏。screen.width+height 本身也被 sdk.js 采集
    orientation: isIOS
