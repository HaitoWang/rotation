...
class MailboxURLProvider(MailProvider):
    ...
    def parse_line(cls, line: str) -> dict:
        parts = [part.strip() for part in str(line or "").split("----", 1)]
        if len(parts) != 2:
            raise ValueError(
    ...
    def wait_for_otp(
        ...
        while self._clock() < deadline:
            ...
            try:
                ...
                # The mailbox page may legitimately change unrelated markup
                # (tracking pixels, refresh hints, generated attributes) while
                # the same newly-arrived message remains visible. Confirmation
                # is therefore based on the code itself, not the whole-page
                # fingerprint. The fingerprint is still used above to reject
