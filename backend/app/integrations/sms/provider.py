...
class SmsPersistenceCallbacks(Protocol):
    """Synchronous callbacks used by the blocking provider to persist state."""
...
def _project_cache_dir() -> Path:
    root = Path(os.getenv("REGERT_DATA_DIR", "")) if os.getenv("REGERT_DATA_DIR") else Path(__file__).resolve().parents[3] / "data"
    cache = root
...
def _track_sms_activation(
    ...
    *, persistence: Optional[SmsPersistenceCallbacks] = None,
) -> None:
    """Persist a rental through the owning async service."""
    if persistence is None:
        logger.warning("SMS rental persistence callback is not configured")
        return
    try:
        persistence.track(platform, activation_id, phone_number, acquired_at)
...
def _queue_sms_activation_cancel(
    ...
    *,
    ...
    if persistence is None:
        logger.warning("SMS cleanup persistence callback is not configured")
        return
    try:
...
def _complete_sms_activation_cleanup(
    platform: str, activation_id: str, *, persistence: Optional[SmsPersistenceCallbacks] = None
) -> None:
    if persistence is None:
        logger.warning("SMS cleanup persistence callback is not configured")
        return
    try:
        persistence.complete(platform, activation_id)
    except Exception as exc:
        logger.warning(
...
class SmsBowerProvider(BaseSmsProvider):
    ...
    def set_persistence(self, persistence: Optional[SmsPersistenceCallbacks]) -> None:
        self.persistence = persistence
                    ...
                    for action in actions:
                        ...
                        while True:
                            ...
                            try:
                                ...
                                track_kwargs = ({"persistence": self.persistence}
                                                if self.persistence is not None else {})
                                _track_sms_activation(
                                    self.platform_key, aid, phone, acquired_at, **track_kwargs
                                )
    ...
    def cancel(self, activation_id: str, *, record_failure: bool = True) -> bool:
        ...
        queue_kwargs = ({"persistence": self.persistence} if self.persistence is not None else {})
        ...
        if ok:
            self.last_cancel_error = ""
            complete_kwargs = ({"persistence": self.persistence}
                               if self.persistence is not None else {})
            _complete_sms_activation_cleanup(self.platform_key, activation_id, **complete_kwargs)
...
class PhoneCallbackController:
    ...
    def _bind_provider_stop(self, provider: BaseSmsProvider) -> None:
        try:
            ...
            pass
        if self.persistence is not None and hasattr(provider, "set_persistence"):
            try:
                provider.set_persistence(self.persistence)
            except Exception:
                pass
