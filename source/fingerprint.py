...
# Chrome (Windows)
# ---------------------------------------------------------------------------
# 只保留有真实 HAR 真值的版本：req/6.27-new/_hdr_chatgpt.txt 实测 Chrome 146。
# 不要凭空添版本——full-version-list 是高熵提示，必须是真实构建号；
# 编造构建号 = 与真实 Chrome 自相矛盾 = 静默死号（见 fingerprint-death-rootcause）。
# brand 串同时与 curl_cffi chrome146 内建值线上实测一致（含品牌顺序）。
_CHROME_VERSIONS = [
    {
        "impersonate": "chrome146",
        "ver": "146",
        # UA 字符串里 Chrome 只发归约版本
        "full_ver": "146.0.0.0",
        # Client Hints 里发真实构建号
        "full_build": "146.0.7680.178",
        "not_a_brand": '"Not-A.Brand";v="24"',
        "not_a_brand_full": '"Not-A.Brand";v="24.0.0.0"',
        # 真实 Chrome 146 的品牌顺序：Chromium, Not-A.Brand, Google Chrome
        "brand_order": ("chromium", "not_a_brand", "chrome"),
    },
]
...
_COUNTRY_PROFILES = {
    ...
    "US": {
        ...
        # 美国出口不得出现 zh-CN：navigator.language(s) 会被 sentinel 采集，
        # 与 US 住宅 IP 跨层矛盾 → 静默标记死号
...
# 兜底策略（未知国家）：代理出口是美国，兜底必须也是美国，
# 绝不能退回 UTC——IP 在美国而时区报 UTC 就是自相矛盾。
...
_LANGUAGES = [
    ...
    ("en-US", "en-US,en;q=0.9,es-US;q=0.8,es;q=0.7"),
...
def _gen_chrome(r: random.Random) -> dict:
    ...
    # 品牌顺序必须与真实 Chrome 一致（146 实测：Chromium, Not-A.Brand, Google Chrome）
    _brand_parts = {
        "chromium": (f'"Chromium";v="{chrome["ver"]}"',
                     f'"Chromium";v="{chrome["full_build"]}"'),
        "chrome": (f'"Google Chrome";v="{chrome["ver"]}"',
                   f'"Google Chrome";v="{chrome["full_build"]}"'),
        "not_a_brand": (chrome["not_a_brand"], chrome["not_a_brand_full"]),
    }
    order = chrome.get("brand_order", ("chromium", "not_a_brand", "chrome"))
    sec_ch_ua = ", ".join(_brand_parts[k][0] for k in order)
    # full-version-list 是高熵提示，发真实构建号；只有 UA 字符串才归约成 146.0.0.0
    sec_ch_ua_full_version_list = ", ".join(_brand_parts[k][1] for k in order)

    # 真实 HAR（req/6.27-new）Windows Chrome 146 报的是 "10.0.0"，
    # 不是 "10.0.19045"，也不是 Win11 的 "15.0.0"。照抄真值。
    ...
    return {
        ...
        "sec_ch_ua_mobile": "?0",
        "sec_ch_ua_full_version": f'"{chrome["full_build"]}"',
...
def generate_fingerprint(rng: random.Random | None = None, country_code: str = "") -> dict:
    ...
    if browser_type != "chrome":
        ...
        fp.setdefault("sec_ch_ua_full_version", "")
...
def fingerprint_for_impersonate(impersonate: str, current_fp: dict) -> dict:
    """同步 TLS fallback 的版本字段，保持 UA 与 Client Hints 一致。

    指纹轮换只改变浏览器版本相关字段；屏幕、语言、时区和硬件仍属于同一
    会话设备，必须保留，否则一次 TLS 重试会同时改变多个可识别维度。
    """
    ...
    if browser_type == "chrome":
        # 与 _gen_chrome 保持同一套品牌顺序 / 真实构建号逻辑
        parts = {
            "chromium": (f'"Chromium";v="{data["ver"]}"',
                         f'"Chromium";v="{data["full_build"]}"'),
            "chrome": (f'"Google Chrome";v="{data["ver"]}"',
                       f'"Google Chrome";v="{data["full_build"]}"'),
            "not_a_brand": (data["not_a_brand"], data["not_a_brand_full"]),
        }
        order = data.get("brand_order", ("chromium", "not_a_brand", "chrome"))
        fp["sec_ch_ua"] = ", ".join(parts[k][0] for k in order)
        fp["sec_ch_ua_full_version_list"] = ", ".join(parts[k][1] for k in order)
        fp["sec_ch_ua_full_version"] = f'"{data["full_build"]}"'
        ...
        # Safari/Firefox 不发送 Chromium Client Hints。
        for key in (
            "sec_ch_ua", "sec_ch_ua_platform", "sec_ch_ua_mobile",
            "sec_ch_ua_full_version_list", "sec_ch_ua_full_version",
