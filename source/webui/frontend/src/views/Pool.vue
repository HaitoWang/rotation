...
onActivated(() => load())
onDeactivated(() => { importVisible.value = false })
loadProviders()
</script>
<template>
  <div class="page">
    <el-card shadow="never">
      <template #header>
        <div class="panel-header">
          <div class="panel-title-group">
            ...
            <p class="section-subtitle">共 {{ total }} 个邮箱账号</p>
          </div>
          <el-button type="primary" @click="importVisible = true"><el-icon><Plus /></el-icon>添加邮箱</el-button>
        </div>
      </template>
      ...
      <div class="selection-toolbar" :class="{ active: selected.length }">
        <span class="selection-count">{{ selected.length ? `已选择 ${selected.length} 项` : '批量操作' }}</span>
      ...
      >
        ...
        <template #empty>
          <el-empty description="暂无邮箱账号" :image-size="70">
            <el-button type="primary" @click="importVisible = true"><el-icon><Plus /></el-icon>添加邮箱</el-button>
          </el-empty>
        </template>
      </el-table>
    ...
    <ImportAccountsDialog v-model="importVisible" @imported="afterImport" />
  </div>
</template>

<style scoped>
.selection-toolbar { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; min-height: 48px; margin-bottom: 14px; padding: 7px 9px; border: 1px solid var(--app-border); border-radius: 9px; background: var(--el-fill-color-lighter); }
.selection-toolbar.active { border-color: color-mix(in srgb, var(--brand) 28%, var(--app-border)); background: var(--brand-soft); }
.selection-count { min-width: 82px; padding-left: 4px; color: var(--el-text-color-secondary); font-size: 11px; }
@media (max-width: 680px) { .selection-toolbar .el-select { flex: 1 1 170px; } }
</style>
