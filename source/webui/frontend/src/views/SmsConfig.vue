...
const strategy = ref('success_first')
const smsbowerEnabled = ref(true)
const heroEnabled = ref(true)
const smsbowerKey = ref('')
const heroKey = ref('')
const smsbowerKeyPh = ref('粘贴 SmsBower API Key')
const heroKeyPh = ref('粘贴 HeroSMS API Key')
...
async function load() {
  try {
    ...
    mode.value = config.sms_mode || 'single'
    strategy.value = config.sms_supplier_strategy || 'success_first'
    smsbowerEnabled.value = config.sms_smsbower_enabled === '1'
    heroEnabled.value = config.sms_herosms_enabled === '1'
    smsbowerKey.value = ''
    heroKey.value = ''
    smsbowerKeyPh.value = config.sms_smsbower_api_key === '***' ? '已设置（留空不修改）' : '粘贴 SmsBower API Key'
    heroKeyPh.value = config.sms_herosms_api_key === '***' ? '已设置（留空不修改）' : '粘贴 HeroSMS API Key'
    apiKey.value = ''
...
async function save() {
  ...
  try {
    await saveSmsConfig({
      ...
      sms_mode: mode.value,
      sms_supplier_strategy: strategy.value,
      sms_smsbower_enabled: smsbowerEnabled.value ? '1' : '0',
      sms_herosms_enabled: heroEnabled.value ? '1' : '0',
      sms_smsbower_api_key: smsbowerKey.value.trim() || '***',
      sms_herosms_api_key: heroKey.value.trim() || '***',
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="运行模式">
          <el-segmented v-model="mode" :options="[
            { label: '单平台', value: 'single' },
            { label: '1:1 分流', value: 'split' },
            { label: '登录后双会话', value: 'session_race' },
          ]" />
        </el-form-item>

        <el-form-item label="号码排序策略">
          <el-radio-group v-model="strategy">
            <el-radio value="success_first">成功率优先</el-radio>
            <el-radio value="balanced">价格与成功率平衡</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item>
              <el-checkbox v-model="heroEnabled">启用 HeroSMS</el-checkbox>
              <el-input v-model="heroKey" type="password" show-password :placeholder="heroKeyPh" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item>
              <el-checkbox v-model="smsbowerEnabled">启用 SmsBower</el-checkbox>
              <el-input v-model="smsbowerKey" type="password" show-password :placeholder="smsbowerKeyPh" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="最多手机号尝试次数（空=默认 3）">
              <el-input v-model="maxPhoneAttempts" type="number" min="1" step="1" placeholder="留空使用默认值 3" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
