...
FORMATS: list[ExportFormat] = [
    ...
    ExportFormat(
        id="team_sso_free_account",
        label="邮箱----密码(gpt)----client_id----refresh_token----2FA----access_token(gpt)----refresh_token(gpt)",
        filename="team-sso-free账号.txt",
        render=lambda r: (
            f'{_s(r, "email")}----{_s(r, "password")}----'
            f'{_s(r, "mail_client_id")}----{_s(r, "mail_refresh_token")}----'
            f'{_s(r, "totp_secret")}----{_s(r, "access_token")}----'
            f'{_s(r, "refresh_token")}'
        ),
        note="含邮箱取码凭证，可直接导入 team-sso free 账号池",
    ),
    ExportFormat(
        id="team_sso_free_account_no_2fa",
        label="邮箱----密码(gpt)----client_id----refresh_token----access_token(gpt)----refresh_token(gpt)",
        filename="team-sso-free账号-无2FA.txt",
        render=lambda r: (
            f'{_s(r, "email")}----{_s(r, "password")}----'
            f'{_s(r, "mail_client_id")}----{_s(r, "mail_refresh_token")}----'
            f'{_s(r, "access_token")}----{_s(r, "refresh_token")}'
        ),
        note="未开启 2FA 的账号",
    ),
]
