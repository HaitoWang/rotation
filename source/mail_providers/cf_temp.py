...
class CFTempEmailProvider(MailProvider):
    ...
    def __init__(
        ...
        if session is not None:
            ...
            # 代理出口是美国：impersonate 配方与注册流程统一（chrome146），
            # 并显式压掉 curl_cffi 内建的 macOS 提示 / 非美语言。
            try:
                from http_client import (
                    DEFAULT_IMPERSONATE, create_http_session, us_chrome_headers,
                )
                us = us_chrome_headers()
                self._session = create_http_session(
                    proxy=None,
                    impersonate=DEFAULT_IMPERSONATE,
                    user_agent=us["User-Agent"],
                    accept_language=us["Accept-Language"],
                    client_hints={k: v for k, v in us.items()
