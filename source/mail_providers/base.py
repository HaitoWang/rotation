...
class MailProvider(ABC):
    ...
    def finalize(self, success: bool = False) -> None:
        """注册流程结束时释放 provider 外部资源；默认无需处理。"""
        return None
...
def parse_import_line(line: str, kind: str = "") -> dict:
    ...
    ⚠️ 自动识别有天然局限：多个号池来源可能使用相同段数，
       段数一样就猜不出来。所以 WebUI 必须让用户显式选来源，
       自动识别只当兜底。
...
def parse_import_text(text: str, kind: str = "") -> list[dict]:
    ...
    for n, line in numbered:
        try:
            ...
            provider_cls = get_provider_class(row.get("kind") or kind)
            expand = getattr(provider_cls, "expand_import_row", None)
            expanded_rows = expand(row) if callable(expand) else [row]
            for expanded in expanded_rows:
                em = (expanded.get("email") or "").lower()
                if em in seen:
